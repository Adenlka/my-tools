...
function md5(input: string): string {
  ...
  // Encode to UTF-8 bytes then treat as latin1 string for the algorithm
  const utf8 = unescape(encodeURIComponent(input));
  return binl2hex(md5core(str2binl(utf8), utf8.length * 8));
}
...
export default function HashGenerator() {
  ...
  const generate = async () => {
    if (!input) return;
    setLoading(true);
    const results: Record<HashAlgo, string> = {
      'MD5': md5(input),
  ...
  return (
    <div className="space-y-5">
      ...
      {(['MD5', 'SHA-1', 'SHA-256', 'SHA-512'] as HashAlgo[]).map(algo => (
        <div key={algo} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
          ...
          <p className="text-xs font-mono text-gray-500 break-all min-h-[1.25rem]">
            {hashes[algo] || <span className="text-gray-300">—</span>}
          </p>
        </div>
      ))}
    </div>
